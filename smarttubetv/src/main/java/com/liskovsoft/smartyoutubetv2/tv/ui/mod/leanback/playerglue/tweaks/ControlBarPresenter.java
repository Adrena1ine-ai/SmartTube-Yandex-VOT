...
class ControlBarPresenter extends Presenter {
    ...
    class ViewHolder extends Presenter.ViewHolder {
        ...
        SparseArray<Presenter> mItemPresenters = new SparseArray<Presenter>();
        ...
        private void bindControlToAction(final int position,
            ...
            Presenter itemPresenter = adapter.getPresenterSelector().getPresenter(item);
            if (itemPresenter == null) {
                itemPresenter = presenter;
            }
            final Presenter controlPresenter = itemPresenter;
            if (vh != null && mItemPresenters.get(position) != controlPresenter) {
                mItemPresenters.get(position).onUnbindViewHolder(vh);
                mControlBar.removeView(vh.view);
                mViewHolders.remove(position);
                mItemPresenters.remove(position);
                vh = null;
            }
            if (vh == null) {
                vh = controlPresenter.onCreateViewHolder(mControlBar);
                mViewHolders.put(position, vh);
                mItemPresenters.put(position, controlPresenter);

                final Presenter.ViewHolder itemViewHolder = vh;
                controlPresenter.setOnClickListener(vh, new View.OnClickListener() {
                    @Override
                ...
                ((ControlButtonPresenterSelector.ControlButtonPresenter) controlPresenter).setOnLongClickListener(vh, new View.OnLongClickListener() {
                    @Override
            ...
            controlPresenter.onBindViewHolder(vh, item);
        }
