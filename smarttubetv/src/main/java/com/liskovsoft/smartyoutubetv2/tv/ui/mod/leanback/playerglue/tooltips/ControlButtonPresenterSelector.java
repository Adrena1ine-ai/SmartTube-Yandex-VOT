...
public class ControlButtonPresenterSelector extends PresenterSelector {
    ...
    private final ControlButtonPresenter mVoiceTranslatePresenter = new ControlButtonPresenter(
            com.liskovsoft.smartyoutubetv2.tv.R.layout.vot_control_button_primary);
    private final Presenter[] mPresenters = new Presenter[]{mPrimaryPresenter, mVoiceTranslatePresenter};

    public ControlButtonPresenterSelector(boolean tooltipsEnabled) {
        mPrimaryPresenter.tooltipsEnabled = mSecondaryPresenter.tooltipsEnabled =
                mVoiceTranslatePresenter.tooltipsEnabled = tooltipsEnabled;
    }
    ...
    public Presenter getPresenter(Object item) {
        return item instanceof VoiceTranslateAction ? mVoiceTranslatePresenter : mPrimaryPresenter;
    }
        ...
        public void onBindViewHolder(ViewHolder viewHolder, Object item) {
            ...
            if (vh.mLabel != null) {
                if (action instanceof VoiceTranslateAction &&
                        ((VoiceTranslateAction) action).isStatusVisible()) {
                    CharSequence status = ((VoiceTranslateAction) action).getStatusLabel();
                    Log.d(TAG, "VOT control bind: index=%s hasStatus=%s",
                            ((VoiceTranslateAction) action).getIndex(), !TextUtils.isEmpty(status));
                    vh.mLabel.setText(status);
                    vh.mLabel.setVisibility(TextUtils.isEmpty(status) ? View.GONE : View.VISIBLE);
